...
let start_publisher = $(config& conf, ZContext ctx)->ZSocket {
  // get a publisher socket
  mut publisher = make_shared<zmq::socket>(*ctx, zmq::socket_type::pub);
...
let start_websocket = $(config& conf)->WebSocket {
  // configure websocket client
  mut ws_conf = websocket::config();
  ws_conf.set_validate_certificates(false);
  mut ws = make_shared<websocket::client>(ws_conf);
  spdlog::info("websocket provisioned");
...
let tick_service = $(config& conf)->void {
  ...
  mut ctx = make_shared<zmq::context>(1);
  mut publisher = start_publisher(conf, ctx);
  mut ws = start_websocket(conf);
